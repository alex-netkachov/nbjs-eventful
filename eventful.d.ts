...
export interface Eventful<E extends EventMap = Record<string | symbol, any[]>> {
  /** Subscribe to an event. Returns an unsubscribe function. */
  on<K extends keyof E>(event: K, fn: Listener<E[K]>): () => boolean;
  /** Subscribe once to an event. Returns an unsubscribe function (called automatically). */
  once<K extends keyof E>(event: K, fn: Listener<E[K]>): () => boolean;
  /** Unsubscribe a previously registered listener. */
  off<K extends keyof E>(event: K, fn: Listener<E[K]>): boolean;
  /** Emit an event synchronously. */
  emit<K extends keyof E>(event: K, ...args: E[K]): void;
  /** Emit an event and wait for all listeners (run in parallel). */
  emitAsync<K extends keyof E>(event: K, ...args: E[K]): Promise<void>;
